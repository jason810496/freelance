...
{
   ListNode<T> *ptr = pos.ptr_;
   
   
   ListNode<T> *nxt= pos.ptr_->next ;
   if( !nxt ) return ;
   
   ListNode<T> *nxt2 = pos.ptr_->next->next;
    delete nxt;
//    pos->ptr_ = nxt2 ;
    if( !nxt2 ){
        delete nxt2;
...
{
    ListNode<T> *tail = pos.ptr_->next ;
    // pos.ptr_->next = other.head_ ;
    ...
    // temp->next = tail;
     ...
     for (SinglyLinkedListConstIterator<T> p = other.Begin(); p != other.End(); ++p) {
        pos.ptr_->next =  new  ListNode<T>( *p );
        // temp = temp->next;
        pos = pos.ptr_->next ;
    }
