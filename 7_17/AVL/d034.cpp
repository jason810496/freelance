...
template<class T, class S>
AVLTreeNode<T,S>* AVLTree<T,S>::search(T key1){
...
int main(){
    ...
    while(right<=n){
        ...
        AVLTreeNode<int,int> *it =M.search( arr[right] ) , *it2 = M.search( arr[left] );
        if ( it->value==0 ) sum++;
        // M[arr[right]]++;
        it->value++;
        right++;
