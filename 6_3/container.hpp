...
std::ostream& operator <<(std::ostream& Out,CS52::Container &other){
    for(int i=0;i<other.size() ; i++){
        Out<<other.at(i)<<" ";
    }
...
CS52::Container CS52::Container::operator -(const CS52::Container &rhs){
    int m=rhs.size();

    Container res( _size , 0);

    for(int i=0;i<m;i++){
        res[i] = _data[i] - rhs.at(i);
    }
